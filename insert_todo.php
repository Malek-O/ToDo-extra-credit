...
function insert_task($task)
{
    ...
    $insert_statement = $GLOBALS['conn']->prepare("INSERT INTO tasks (task, createdAt, done) VALUES(?, now(),0);");
    if ($insert_statement) {
        ...
        */
        if (!$insert_statement->execute() || $insert_statement->affected_rows !=1) {
            print_r('Error executing MySQL insert statement: ' . $insert_statement->error);
            return;
        }
        ...
        $insert_statement->errno, $insert_statement->error);
    }
}
